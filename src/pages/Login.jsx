...
import { Lock, User, Server, Eye, EyeOff, ShieldCheck, ArrowRight } from 'lucide-react';
...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Fallback offline demo login if backend is not running
      if (username === 'admin' && password === 'admin') {
        const demoUser = { id: 1, username: 'admin', role_name: 'Administrator', dept_name: 'Operations' };
        localStorage.setItem('user', JSON.stringify(demoUser));
        setAuthToken('demo-token-' + Date.now());
        navigate('/');
      } else {
        setError('Cannot reach server (Demo login: username "admin", password "admin")');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4 relative overflow-hidden font-sans">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-32 -right-32 w-96 h-96 bg-emerald-900/20 rounded-full blur-3xl" />
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white tracking-tight">{company?.name || 'Northstar AssetOps'}</h1>
          <p className="text-slate-400 text-sm mt-1">Enterprise Inventory Suite</p>
        </div>

        {/* Login Card */}
        <div className="bg-white rounded-3xl shadow-2xl p-8 border border-slate-200">
          <h2 className="text-xl font-bold text-slate-900 mb-1">Sign In</h2>
          <p className="text-sm text-slate-500 mb-6">Enter your credentials to access the inventory console</p>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div>
              ...
              >
                <Server size={14} />
                {showConfig ? 'Hide server settings' : 'Backend Server settings'}
              </button>
              {showConfig && (
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full rounded-xl bg-slate-900 py-3 text-sm font-semibold text-white hover:bg-slate-800 active:scale-[0.99] transition shadow-soft disabled:opacity-50 flex items-center justify-center gap-2"
            >
        ...
        <div className="flex items-center justify-center gap-2 mt-5 text-slate-500 text-xs">
          ...
          <span>Local & Offline Secure Authentication Engine</span>
        </div>
      </div>
    </div>
  );

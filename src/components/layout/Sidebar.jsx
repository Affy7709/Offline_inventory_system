    ...
    >
      <div className="flex items-center justify-between border-b border-slate-800 px-4 py-4">
        <div className="flex items-center gap-3 overflow-hidden">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-accent-500/20 text-sm font-bold text-accent-100">{company.short}</div>
          {(!collapsed || mobileOpen) && <div>
            <div className="text-sm font-semibold text-white">{company.name}</div>
            <div className="text-[10px] uppercase tracking-[0.2em] text-slate-400">Inventory Suite</div>
          </div>}
        </div>
      ...
      <nav className="flex-1 space-y-1 px-3 py-4 overflow-y-auto">
        {navigation.map((item) => {
          ...
          return (
            ...
            >
              <Icon size={18} />
              {(!collapsed || mobileOpen) ? <span>{item.name}</span> : <span className="md:hidden">{item.name}</span>}
            </NavLink>
          )
        })}
      </nav>
